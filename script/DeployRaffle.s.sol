...
contract DeployRaffle is Script {
	...
	function run() external returns (Raffle, HelperConfig) {
		...
		// Below is equvalent to above but you need to assign it 1 by 1 for each field of the struct
		// NetworkConfig config = helperConfig.activeNetworkConfig();
		// uint256 entranceFee = config.entranceFee;
		// and goes on ...

		if(subscriptionId == 0){	// If we dont have a subscription, create one & fund
